import sys
sys.path.insert(0,'/storage/Ayantika/analyse_1/Rashmi/Swin-MAE/')
import os
import numpy as np
import torch
...
class SwinMAE(nn.Module):
    ...
    def __init__(self, img_size: int = 224, patch_size: int = 4, mask_ratio: float = 0.25, in_chans: int = 3,
                 decoder_embed_dim=768, norm_pix_loss=False,
                 depths: tuple = (2, 2, 2, 2), embed_dim: int = 96, num_heads: tuple = (3, 6, 12, 24),
    ...
    def window_masking(self, x: torch.Tensor,window_arr, r: int = 4,
                       remove: bool = False, mask_len_sparse: bool = False, index = 27):
        """
...
#         sparse_keep = sparse_shuffle[:, :int(d ** 2 * (1 - self.mask_ratio))]  #commented by Rashmi
        ...
        arr = [*range(0, 196, 1)]  
        arr2 = list(set(arr)-set(window_arr))
        sparse_keep = torch.tensor([arr2])        

# ************************above code added by Rashmi******************************************            
...
#****************************************************************************************************************************
    ...
    def forward_encoder(self, x,window_arr):
        ...
        x, mask = self.window_masking(x,window_arr, remove=False, mask_len_sparse=False)
