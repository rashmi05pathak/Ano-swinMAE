...
path_nii = '/storage/Ayantika/Data_final/brats21/**/**t1.nii.gz'
path_h5 = '/storage/Ayantika/analyse_1/Rashmi/brain_cache/h5_data/Brats21_T1'
...
mask_nii = '/storage/Ayantika/Data_final/brats21/**/**seg.nii.gz'
...
test_loader = torch.utils.data.DataLoader(h5cacheds,batch_size = 5,shuffle = True)
...
# def curate_dataloader(_dataset, batch_size,shuffle):
#     include_indices = []
#     for i, batch in tqdm(enumerate(_dataset)): 
# #         print(batch['_train_label'].shape)
#         if torch.sum(batch['label'])!=0:
# #             if (batch['slicenum']>=80) and (len(include_indices)< 1010) and (batch['slicenum']<=100): 
#             include_indices.append(i)
# #             print('j')
                

    
#     included_subset = Subset(_dataset, include_indices)
#      # Create a DataLoader for the included elements
#     included_data_loader = DataLoader(included_subset, batch_size=batch_size, shuffle=shuffle)
            

#     return included_subset,included_data_loader


# In[5]:


# _, test_loader = curate_dataloader(h5cacheds, 5,True)
# for i, batch in enumerate(test_loader):
#     for b in range(0,len(batch['filenum'])):
#         if torch.sum(batch['label'][b])==0:
#             print('j')
# #     break
...
model = swin_mae_inference.SwinMAE()
...
for i, batch_ in enumerate(test_loader):
    ...
    for j in range(0,batch_size):
        if(np.sum(batch_['label'][j,0,:,:].numpy())>10): #sending only mri slices with pathology
...
for key in dict_comb:s
    print(dict_comb[key]['dice_score'])
    if dict_comb[key]['dice_score']>0:
        print(dict_comb[key]['dice_score'])
...
sum(dice_accum_without_zero)/len(dice_accum_without_zero)


# In[ ]:


len(dice_accum_without_zero)
...
for key in dict_comb:
    dice_accum.append(dict_comb[key]['dice_score'])


# In[14]:


sum(dice_accum)/len(dice_accum)
